...
```
<b>'api/v1/buddy/student-check/'</b> - Проверка наличия приезда у студента. POST запрос в виде:
```
{
    "student_id": "1"
}
```
student_id - id студента, у которого проверяем наличие приезда<br>
Выдает True если студент имеет приезд и False если не имеет<br>
<h2>Тимлид</h2><br>
...
```
id - id сопровождающего<br>
full_name - имя сопровождающего<br>
arrival_count - количество подтвержденных приездов у сопровождающего<br>
<b>'api/v1/teamlead/not-confirmed-buddys/'</b> - Просмотр тимлидом всех неподтвержденных сопровождающих. GET запрос выдает ответ в виде:
```
[
    {
        "user": {
...
```
id - id сопровождающего<br>
full_name - имя сопровожлающего<br>
<br>
<h2>Мессенджер</h2><br>
